            ...
            <thead class="text-gray-600 border-b bg-gray-50/70">
                <tr>
                    ...
                    <th class="p-3 text-left cursor-pointer" wire:click="sortBy('no_surat')">
                        No Surat
                        {!! $sortField === 'no_surat' ? ($sortDirection === 'asc' ? '⬆️' : '⬇️') : '' !!}
                    </th>
                    ...
                    <th class="p-3 text-left cursor-pointer w-32" wire:click="sortBy('tanggal')">
                        Tanggal
                        {!! $sortField === 'tanggal' ? ($sortDirection === 'asc' ? '⬆️' : '⬇️') : '' !!}
                    </th>
                    <th class="p-3 text-left w-32">File</th>
                    <th class="p-3 text-left w-32">Creator</th>
                    <th class="p-3 text-left w-32">Creator Role</th>
                    <th class="p-3 text-left w-32">Updater</th>
                    <th class="p-3 text-left w-32">Updater Role</th>
            ...
            <tbody class="text-gray-700">
                @forelse ($surats as $index => $surat)
                    <tr class="border-b hover:bg-gray-50/70 transition">
                        ...
                        {{-- Creator --}}
                        <td class="p-3">{{ optional($surat->creator)->name ?? '-' }}</td>

                        {{-- Creator Role --}}
                        <td class="p-3">{{ optional(optional($surat->creator)->role)->name ?? '-' }}</td>

                        {{-- Updater --}}
                        <td class="p-3">{{ optional($surat->updater)->name ?? '-' }}</td>

                        {{-- Updater Role --}}
                        <td class="p-3">{{ optional(optional($surat->updater)->role)->name ?? '-' }}</td>

                        {{-- Aksi --}}
                        <td class="p-3 flex gap-2 flex-wrap">
                            <a href="{{ route('disposisi.management', $surat->id) }}"
            ...
            @if($isDisposisiMinimized)
                <div class="fixed bottom-6 left-6 z-50">
                    <div class="flex items-center gap-3 bg-white/90 backdrop-blur-md rounded-full px-3 py-2 shadow-lg border border-gray-200 cursor-pointer"
                        wire:click="restoreDisposisi">
                        <div class="flex items-center gap-2">
                            <button wire:click.stop="closeDisposisi"
                                class="w-3 h-3 rounded-full bg-red-500 hover:bg-red-600"></button>
                            <button wire:click.stop="restoreDisposisi"
                                class="w-3 h-3 rounded-full bg-yellow-400 hover:bg-yellow-500"></button>
                            <button wire:click.stop="toggleDisposisiFullscreen"
                                class="w-3 h-3 rounded-full bg-green-500 hover:bg-green-600"></button>
                        </div>
                        <span class="ml-3 text-sm font-medium text-gray-800">
                            📑 Disposisi Surat #{{ $selectedSuratId }}
                        </span>
                    </div>
                </div>
            @else
                <div
                    class="@if($isDisposisiFullscreen) w-full max-w-none h-[90vh] max-h-[90vh] rounded-xl @else w-full max-w-3xl rounded-3xl @endif
                    ...
                    <div class="flex items-center justify-between px-3 py-2 bg-gray-50 border-b">
                        <div class="flex items-center space-x-2">
                            <button wire:click="closeDisposisi"
                                class="w-3 h-3 rounded-full bg-red-500 hover:bg-red-600"></button>
                            <button wire:click="minimizeDisposisi"
                                class="w-3 h-3 rounded-full bg-yellow-400 hover:bg-yellow-500"></button>
                            <button wire:click="toggleDisposisiFullscreen"
                                class="w-3 h-3 rounded-full bg-green-500 hover:bg-green-600"></button>
                        </div>
                    ...
                    <div class="p-4 sm:p-6 overflow-auto h-full">
                        <livewire:disposisi.disposisi-management :suratMasukId="$selectedSuratId" />
                    </div>
                </div>
            @endif
        </div>
                    ...
                    <div class="p-4 sm:p-6 overflow-auto h-full">
                        <form wire:submit.prevent="store" class="space-y-4">
                            ...
                            >
                                ...
                                {{-- Drop area --}}
                                <div class="relative flex flex-col items-center justify-center w-full border-2 border-dashed rounded-xl cursor-pointer transition p-6"
                                ...
                                {{-- Progress bar --}}
                                <div wire:loading wire:target="file_surat"
